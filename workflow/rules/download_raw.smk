...
rule download_from_ERDA:
    input:
        ready=os.path.join(config["workdir"], "ERDA_folder_created"),
        filesize=os.path.join(config["workdir"], "{sample}_filesize.txt"),
    output:
        r1o=temp(os.path.join(config["workdir"], "{sample}_raw_1.fq.gz")),
        r2o=temp(os.path.join(config["workdir"], "{sample}_raw_2.fq.gz")),
    conda:
