...
rule upload_to_ERDA:
    input:
        ...
        host_bam=os.path.join(config["workdir"], "{sample}_G.bam"),
        coverm=os.path.join(config["workdir"], "misc/{sample}_coverM_mapped_host.tsv"),
        file_size=os.path.join(config["workdir"], "{sample}_filesize.txt"),
    output:
        os.path.join(config["workdir"], "misc/{sample}_uploaded"),
    conda:
